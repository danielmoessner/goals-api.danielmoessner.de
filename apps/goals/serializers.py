...
class StrategySerializer(serializers.HyperlinkedModelSerializer):
    ...
    goal = serializers.HyperlinkedRelatedField(
        many=False, view_name="goal-detail", queryset=Goal.objects.none()
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["goal"].queryset = Goal.get_goals_user(
            user, "ALL", include_archived_goals=user.show_archived_objects
        )
...
class LinkSerializer(serializers.HyperlinkedModelSerializer):
    ...
    id = serializers.ReadOnlyField()
    master_goal = serializers.HyperlinkedRelatedField(
        many=False, view_name="goal-detail", queryset=Goal.objects.none()
    )
    sub_goal = serializers.HyperlinkedRelatedField(
        many=False, view_name="goal-detail", queryset=Goal.objects.none()
    )
